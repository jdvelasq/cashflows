...
def cashflow(const_value=0, start=None, end=None, periods=None, freq='A'):
    ...
    """
    if freq not in ['A', 'BA', 'Q', 'BQ', 'M', 'BM', 'CBM', 'SM']:
        msg = 'Invalid freq value:  ' + freq.__repr__()
        raise ValueError(msg)
    if isinstance(const_value, list):
        periods = len(const_value)

    prng = pd.date_range(start=start, end=end, periods=periods, freq=freq)
    periods = len(prng)
...
def interest_rate(const_value=0, start=None, end=None, periods=None, freq='A', chgpts=None):
    ...
    >>> interest_rate(const_value=[10]*10, start='2000Q1', freq='Q')  # doctest: +NORMALIZE_WHITESPACE
    2000-03-31    10.0
    2000-06-30    10.0
    2000-09-30    10.0
    2000-12-31    10.0
